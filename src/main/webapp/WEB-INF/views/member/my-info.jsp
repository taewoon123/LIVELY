      ...
      <main>
        ...
        <div id="" class="row g-5">
                        ...
                        <div class="selections">
                          <select class="location-option" name="locationNo" id="location">
                            <c:forEach items="${locationList}" var="locationList">
                              <option value="${locationList.getLocationNo()}">${locationList.getLocationName()}
                              </option>
                            </c:forEach>
                          </select>
                        </div>
          ...
          <div class="col-md-5 col-lg-4">
            ...
            <ul class="list-group mb-3">
              <div class="btn-group-vertical" role="group" aria-label="Vertical button group">
                <button type="button" class="btn" onclick="location.href='${rootContext}/member/my-info'">나의 정보
                  수정</button>
                <button type="button" class="btn" onclick="location.href='${rootContext}/member/my-board'">내가 쓴
                  글</button>
                <button type="button" class="btn"
                  onclick="location.href='${rootContext}/member/my-friend-feed'">내가 쓴 피드</button>
              </div>
              <br>
              <button type="button" class="btn" id="quit-button">회원 탈퇴하기</button>
            </ul>
          </div>
          <!-- //////////////////위까지 사이드바////////////////// -->
          ...
          <script>
            ...
            const rootContext = '<%= request.getContextPath() %>';

            quitButton.addEventListener('click', () => {
              const quitYn = confirm('정말 탈퇴하시겠습니까?');
              if (quitYn) {
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = rootContext + '/member/quit';
